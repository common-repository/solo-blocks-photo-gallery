...
class Grid extends Basic {
	...
	protected function render($settings){
		...
		if(isset($settings['images']) && is_array($settings['images']) && count($settings['images'])) {
			$this->add_responsive_block('.solo_isotope_wrapper', 'margin-right:-%1$spx; margin-bottom:-%1$spx;', $settings['grid_gap']);
			$this->add_responsive_block('.isotope_item', 'padding-right: %1$spx; padding-bottom: %1$spx;', $settings['grid_gap']);
			...
			$this->add_render_attribute('wrapper', 'data-settings', wp_json_encode(array(
				...
				'load_more'      => esc_attr($settings['load_more']),
				'load_items'     => esc_attr($settings['load_more_count']),
